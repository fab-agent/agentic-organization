...
def _personnel_to_dict(p: Personnel, session) -> dict:
    ...
    cfg = session.exec(
        select(AgentConfig).where(AgentConfig.personnel_id == p.id)
    ).first()
    ...
    if cfg:
        responsible = (
            session.get(Personnel, cfg.responsible_id) if cfg.responsible_id else None
        )
...
def _get_person_and_check_membership(
    person_id: str, user_id: str, session
) -> Personnel:
...
def create_personnel(
    body: PersonnelCreate, current_user: User = Depends(get_current_user)
):
    with get_session() as session:
        if body.company_id:
...
def invite_personnel(
    person_id: str, body: dict, current_user: User = Depends(get_current_user)
):
    ...
    with get_session() as session:
        ...
        if not person.email:
            raise HTTPException(
                status_code=422, detail="Bu personelin e-posta adresi yok"
            )
        if person.type != "human":
            raise HTTPException(
                status_code=422, detail="Sadece insan personele davet gönderilebilir"
            )
...
def archive_personnel(person_id: str, current_user: User = Depends(get_current_user)):
    ...
    with get_session() as session:
        person = _get_person_and_check_membership(person_id, current_user.id, session)
        cfg = session.exec(
            select(AgentConfig).where(AgentConfig.personnel_id == person_id)
        ).first()
        if cfg:
            cfg.status = "inactive"
...
def update_agent_config(
    ...
    with get_session() as session:
        ...
        if body.responsible_id is not None:
            cfg.responsible_id = body.responsible_id
        cfg.updated_at = datetime.utcnow()
        session.add(cfg)
        log_action(
            session,
...
def add_skill(
    person_id: str, body: SkillCreate, current_user: User = Depends(get_current_user)
):
    with get_session() as session:
        _get_person_and_check_membership(person_id, current_user.id, session)
        cfg = session.exec(
        ...
        session.add(skill)
        log_action(
            session, "create", "skill", entity_id=skill.id, entity_name=skill.name
        )
        session.commit()
...
def update_skill(
    ...
    with get_session() as session:
        ...
        if body.config is not None:
            skill.config_json = _json.dumps(body.config)
        if body.is_active is not None:
            skill.is_active = body.is_active
        session.add(skill)
        log_action(
            session, "update", "skill", entity_id=skill.id, entity_name=skill.name
        )
        session.commit()
        session.refresh(skill)
        return _skill_to_dict(skill)
...
def delete_skill(
    person_id: str, skill_id: str, current_user: User = Depends(get_current_user)
):
    with get_session() as session:
        _get_person_and_check_membership(person_id, current_user.id, session)
        skill = session.get(Skill, skill_id)
        if not skill:
            raise HTTPException(status_code=404, detail="Skill not found")
        log_action(
            session, "delete", "skill", entity_id=skill.id, entity_name=skill.name
        )
...
def set_agent_policies(
    person_id: str, body: PolicyLinkSet, current_user: User = Depends(get_current_user)
):
    ...
    with get_session() as session:
        ...
        log_action(
            session, "update", "agent_config", entity_id=cfg.id, entity_name=person.name
        )
        session.commit()
...
def get_org_tree(
    ...
    with get_session() as session:
        ...
        linked_rows = session.exec(
            select(AgentSkillLink, CompanySkill).join(
                CompanySkill, AgentSkillLink.company_skill_id == CompanySkill.id
            )
        ).all()
        ...
        def build_node(p: Personnel) -> dict:
            ...
            if cfg:
                ...
                node["dept_policies"] = (
                    dept_pols  # [{id, name}] — inherited, not editable
                )
                node["agent_policies"] = agent_pols  # [{id, name}] — agent-specific
                node["policies"] = [p["name"] for p in dept_pols] + [
                    p["name"] for p in agent_pols
                ]
